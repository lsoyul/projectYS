        ...
        protected override void DoLevel()
        {
            ...
            switch (forceDriver.forceType)
            {
                case ForceType.Force:
                    rb.AddForce(travel, forceDriver.forceMode);
                    break;

                case ForceType.RelativeForce:
                    rb.AddRelativeForce(travel, forceDriver.forceMode);
                    break;

                case ForceType.Torque:
                    rb.maxAngularVelocity = forceDriver.maxAngularVelocity;
                    rb.AddTorque(travel, forceDriver.forceMode);
                    break;

                case ForceType.RelativeTorque:
                    rb.maxAngularVelocity = forceDriver.maxAngularVelocity;
                    rb.AddRelativeTorque(travel, forceDriver.forceMode);
                    break;
            }
        }
    }
